...
def _query_graph_text(
    ...
    *,
    ...
    # Traverse undirected (upstream #29b7a21) so a seed with no outgoing edges
    # still reaches its callers; the view carries `_src`/`_tgt` per edge so the
    # renderer keeps caller->callee direction. Built once and shared by the
    # filtered traversal and the relaxed retraversal below.
    view = _traversal_view(G)
    traversal_graph = _filter_graph_by_context(view, resolved_filters)
    ...
    if relaxed:
        traversal_graph = view
        nodes, edges = traverse(view, start_nodes, depth)
    header_parts = [
